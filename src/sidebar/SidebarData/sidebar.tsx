...
import { DataTree, DataTreeProps } from './DataTree'
...
export const SidebarData: SidebarComponent<RenderableDataSidebar> = ({ sidebar }) => {
  const { data: { data, attributes }, title, config, dynamicAttributes } = sidebar;
  ...
  const setTree = React.useCallback< DataTreeProps[ 'setTree' ] >(
    cb => setTreeRaw(prev => cb(prev)),
    [setTreeRaw]
  );
